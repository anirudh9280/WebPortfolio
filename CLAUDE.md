...
- **`commits.csv`** — every commit in the repo, from `git log` via `scripts/gen-commits.mjs`. The hero stat readout uses this.
- **`loc.csv`** — a per-line `git blame`, from `elocuent`. The `/analytics` page uses this.

They disagree, on purpose. `loc.csv` only sees commits whose lines still survive, so a large rewrite collapses it (27 commits in the log, 11 in the blame). Don't "fix" that by pointing them at the same source: the hero counts commits and needs the log, while the analytics page is about the current codebase's lines and needs the blame. The analytics page labels its figure "Commits in blame" for this reason.
...
The site is built from the vernacular of measurement: axis rails, tick marks, monospace readouts of real numbers. The signature elements are the left-edge axis rail and the hero's animated contour field, which runs marching squares (`d3-contour`) over a noise field banded in `d3.interpolateViridis`.
